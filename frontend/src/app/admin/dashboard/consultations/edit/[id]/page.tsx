...
export default function EditConsult({ params }:{params:{id:string}}) {
  ...
  useEffect(() => {
    if (id) {
      axios.get(`http://localhost:8000/api/v1/consults/${id}`)
        .then(response => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:8000/api/v1/consults/${id}`,
        formData,
        {
      ...
      if (response.data.status === "success") {
        toast({
          description: "Consult updated successfully",
        });
        setSaveDialogOpen(true);
      }
    } catch (error) {
      toast({
        description: "Error updating Consult",
        variant: "destructive",
      })
      console.log("Error updating Consult", error);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="w-full flex flex-col items-center justify-center">
        <h1 className="text-3xl font-bold mb-3">Edit {ConsultData?.title_EN}</h1>
        <div>
        <Form {...form}>
            ...
            {showCoverImageInput ? (
                ...
                ConsultData && (
                  <div className="mt-5">
                      ...
                      <Image 
                        src={`/imgs/${ConsultData.coverImage}`} 
                ...
                ConsultData && (
                  <div className="mt-5 w-full">
                    ...
                    <div className="flex flex-row justify-between ">
                      {ConsultData.images.length > 0 ? (
                        ConsultData.images.map((image, index) => (
                          <Image 
                            key={index}
                            src={`/imgs/${image}`} 
                            alt={`Consult image ${index + 1}`} 
                            width={400} 
                            height={250} 
